...
class app(Frame):
    def __init__(self,root=Tk(),width=364,height=425):
        ...
        display=StringVar()
        Entry(self,relief=RIDGE,textvariable=display,state=DISABLED,justify="right",bd=20,bg="silver").pack(expand=YES,fill=BOTH)
        clrChar="clear"
        ...
        for btnNum in ("789/","456*","123-","0.+"):
            ...
            for fEquals in btnNum:
                button(FunctionNum,LEFT,fEquals,lambda appObj=display,i=fEquals:appObj.set(appObj.get()+i))
                EqualsButton = fCalc(self,TOP)

        for fEquals in "=":
            if fEquals == "=":
                btnEquals = button(EqualsButton,LEFT,fEquals)
                btnEquals.bind('<ButtonRelease-1>',lambda e, s=self,appObj=display:s.result(appObj),"+")
